...
const PermissaoUsuarioPainel = () => {
    ...
    const handleListarUsuariosPorAgente = async () => {
      try {
        ...
        const response = await fetch(
          `http://192.168.5.239:3000/agentes/${agenteId}/usuarios`,
          {
            method: "GET",
            headers: {
  ...
  const togglePermissaoUsuario = async (usuarioId: number, isChecked: boolean) => {
    try {
      ...
      const url = isChecked
        ? `http://192.168.5.239:3000/agentes/${usuarioId}/habilitar`
        : `http://192.168.5.239:3000/agentes/${usuarioId}/desabilitar`;
  ...
  const desmarcarTodos = async () => {
    try {
      ...
      for (const usuario of usuarios) {
        await fetch(`http://192.168.5.239:3000/agentes/${usuario.id}/desabilitar`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
